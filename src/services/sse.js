...
export async function streamDoctorMessage(chatId, text, onEvent) {
  const token = Cookie.get("authToken");
  const res = await fetch(`${BASE_URL}/api/doctor/chats/${chatId}/messages`, {
    method: "POST",
    headers: {
    ...
    while (true) {
      ...
      for (const raw of chunks) {
        ...
        try {
          ...
          console.warn("[DoctorSSE] parse failure", line, err);
        }
      }
    }
  } catch (err) {
